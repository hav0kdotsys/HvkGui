...
//    | TableSetupDrawChannels()                - setup HvkDrawList channels
...
// Default value is HvkVec2(0.0f, 0.0f).
//   X
...
// Hvkportant to note how the two flags have slightly different behaviors!
//   - HvkGuiTableFlags_NoHostExtendX -> Make outer width auto-fit to columns (overriding outer_size.x value). Only available when ScrollX/ScrollY are disabled and Stretch columns are not used.
//   - HvkGuiTableFlags_NoHostExtendY -> Make outer height stop exactly at outer_size.y (prevent auto-extending table past the limit). Only available when ScrollX/ScrollY is disabled. Data below the limit will be clipped and not visible.
// In theory HvkGuiTableFlags_NoHostExtendY could be the default and any non-scrolling tables with outer_size.y != 0.0f would use exact height.
...
// (Reference: HvkGuiTableFlags_SizingXXX flags and HvkGuiTableColumnFlags_WidthXXX flags)
//-----------------------------------------------------------------------------
// About overriding column sizing policy and width/weight with TableSetupColumn():
// We use a default parameter of -1 for 'init_width'/'init_weight'.
//   - with HvkGuiTableColumnFlags_WidthFixed,    init_width  <= 0 (default)  --> width is automatic
//   - with HvkGuiTableColumnFlags_WidthFixed,    init_width  >  0 (explicit) --> width is custom
//   - with HvkGuiTableColumnFlags_WidthStretch,  init_weight <= 0 (default)  --> weight is 1.0f
//   - with HvkGuiTableColumnFlags_WidthStretch,  init_weight >  0 (explicit) --> weight is custom
// Widths are specified _without_ CellPadding. If you specify a width of 100.0f, the column will be cover (100.0f + Padding * 2.0f)
// and you can fit a 100.0f wide item in it without clipping and with padding honored.
//-----------------------------------------------------------------------------
// About default sizing policy (if you don't specify a HvkGuiTableColumnFlags_WidthXXXX flag)
//   - with Table policy HvkGuiTableFlags_SizingFixedFit      --> default Column policy is HvkGuiTableColumnFlags_WidthFixed, default Width is equal to contents width
//   - with Table policy HvkGuiTableFlags_SizingFixedSame     --> default Column policy is HvkGuiTableColumnFlags_WidthFixed, default Width is max of all contents width
//   - with Table policy HvkGuiTableFlags_SizingStretchSame   --> default Column policy is HvkGuiTableColumnFlags_WidthStretch, default Weight is 1.0f
//   - with Table policy HvkGuiTableFlags_SizingStretchWeight --> default Column policy is HvkGuiTableColumnFlags_WidthStretch, default Weight is proportional to contents
...
//   - when using HvkGuiTableFlags_SizingFixedSame with mixed columns, only the Fixed/Auto columns will match their widths to the width of the maximum contents.
//   - when using HvkGuiTableFlags_SizingStretchSame with mixed columns, only the Stretch columns will match their weights/widths.
//-----------------------------------------------------------------------------
...
// - For large numbers of rows, it is recommended you use HvkGuiListClipper to submit only visible rows.
//   HvkGuiListClipper is reliant on the fact that rows are of equal height.
...
//  HvkDrawList output:    normal      dummy        dummy       -> [internal] when using the dummy channel, HvkDrawList submissions (if any) will be wasted (because cliprect is zero-width anyway).
//
...
static const int TABLE_DRAW_CHANNEL_NOCLIP = 2;                     // When using HvkGuiTableFlags_NoClip (this becomes the last visible channel)
static const float TABLE_BORDER_SIZE                     = 1.0f;    // FIXME-TABLE: Currently hard-coded because of clipping assumptions with outer borders rendering.
static const float TABLE_RESIZE_SEPARATOR_HALF_THICKNESS = 4.0f;    // Extend outside inner borders.
static const float TABLE_RESIZE_SEPARATOR_FEEDBACK_TIMER = 0.06f;   // Delay/timer before making the hover feedback (color+cursor) visible because tables/columns tends to be more cramped.

// Helper
inline HvkGuiTableFlags TableFixFlags(HvkGuiTableFlags flags, HvkGuiWindow* outer_window)
{
    // Adjust flags: set default sizing policy
    if ((flags & HvkGuiTableFlags_SizingMask_) == 0)
        flags |= ((flags & HvkGuiTableFlags_ScrollX) || (outer_window->Flags & HvkGuiWindowFlags_AlwaysAutoResize)) ? HvkGuiTableFlags_SizingFixedFit : HvkGuiTableFlags_SizingStretchSame;

    // Adjust flags: enable NoKeepColumnsVisible when using HvkGuiTableFlags_SizingFixedSame
    if ((flags & HvkGuiTableFlags_SizingMask_) == HvkGuiTableFlags_SizingFixedSame)
        flags |= HvkGuiTableFlags_NoKeepColumnsVisible;

    // Adjust flags: enforce borders when resizable
    if (flags & HvkGuiTableFlags_Resizable)
        flags |= HvkGuiTableFlags_BordersInnerV;

    // Adjust flags: disable NoHostExtendX/NoHostExtendY if we have any scrolling going on
    if (flags & (HvkGuiTableFlags_ScrollX | HvkGuiTableFlags_ScrollY))
        flags &= ~(HvkGuiTableFlags_NoHostExtendX | HvkGuiTableFlags_NoHostExtendY);

    // Adjust flags: NoBordersInBodyUntilResize takes priority over NoBordersInBody
    if (flags & HvkGuiTableFlags_NoBordersInBodyUntilResize)
        flags &= ~HvkGuiTableFlags_NoBordersInBody;

    // Adjust flags: disable saved settings if there's nothing to save
    if ((flags & (HvkGuiTableFlags_Resizable | HvkGuiTableFlags_Hideable | HvkGuiTableFlags_Reorderable | HvkGuiTableFlags_Sortable)) == 0)
        flags |= HvkGuiTableFlags_NoSavedSettings;

    // Inherit _NoSavedSettings from top-level window (child windows always have _NoSavedSettings set)
    if (outer_window->RootWindow->Flags & HvkGuiWindowFlags_NoSavedSettings)
        flags |= HvkGuiTableFlags_NoSavedSettings;

    return flags;
}

HvkGuiTable* HvkGui::TableFindByID(HvkGuiID id)
{
...
bool    HvkGui::BeginTable(const char* str_id, int columns_count, HvkGuiTableFlags flags, const HvkVec2& outer_size, float inner_width)
{
    HvkGuiID id = GetID(str_id);
    return BeginTableEx(str_id, id, columns_count, flags, outer_size, inner_width);
}

bool    HvkGui::BeginTableEx(const char* name, HvkGuiID id, int columns_count, HvkGuiTableFlags flags, const HvkVec2& outer_size, float inner_width)
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiWindow* outer_window = GetCurrentWindow();
    ...
    if (++g.TablesTempDataStacked > g.TablesTempData.Size)
        g.TablesTempData.resize(g.TablesTempDataStacked, HvkGuiTableTempData());
    HvkGuiTableTempData* temp_data = table->TempData = &g.TablesTempData[g.TablesTempDataStacked - 1];
    temp_data->TableIndex = table_idx;
    table->DrawSplitter = &table->TempData->DrawSplitter;
    table->DrawSplitter->Clear();

    // Fix flags
    table->IsDefaultSizingPolicy = (flags & HvkGuiTableFlags_SizingMask_) == 0;
    flags = TableFixFlags(flags, outer_window);

    // Initialize
    const int previous_frame_active = table->LastFrameActive;
    const int instance_no = (previous_frame_active != g.FrameCount) ? 0 : table->InstanceCurrent + 1;
    const HvkGuiTableFlags previous_flags = table->Flags;
    table->ID = id;
    table->Flags = flags;
    ...
    table->NavLayer = (HvkS8)outer_window->DC.NavLayerCurrent;
    temp_data->UserOuterSize = outer_size;

    // Instance data (for instance 0, TableID == TableInstanceID)
    HvkGuiID instance_id;
    ...
    if (use_child_window)
    {
        ...
        HvkVec2 override_content_size(FLT_MAX, FLT_MAX);
        if ((flags & HvkGuiTableFlags_ScrollX) && !(flags & HvkGuiTableFlags_ScrollY))
        ...
        if ((flags & HvkGuiTableFlags_ScrollX) && inner_width > 0.0f)
            override_content_size.x = inner_width;

        if (override_content_size.x != FLT_MAX || override_content_size.y != FLT_MAX)
            SetNextWindowContentSize(HvkVec2(override_content_size.x != FLT_MAX ? override_content_size.x : 0.0f, override_content_size.y != FLT_MAX ? override_content_size.y : 0.0f));

        // Reset scroll if we are reactivating it
        if ((previous_flags & (HvkGuiTableFlags_ScrollX | HvkGuiTableFlags_ScrollY)) == 0)
            if ((g.NextWindowData.HasFlags & HvkGuiNextWindowDataFlags_HasScroll) == 0)
                SetNextWindowScroll(HvkVec2(0.0f, 0.0f));

        // Create scrolling region (without border and zero window padding)
        HvkGuiChildFlags child_child_flags = (g.NextWindowData.HasFlags & HvkGuiNextWindowDataFlags_HasChildFlags) ? g.NextWindowData.ChildFlags : HvkGuiChildFlags_None;
        HvkGuiWindowFlags child_window_flags = (g.NextWindowData.HasFlags & HvkGuiNextWindowDataFlags_HasWindowFlags) ? g.NextWindowData.WindowFlags : HvkGuiWindowFlags_None;
        if (flags & HvkGuiTableFlags_ScrollX)
            child_window_flags |= HvkGuiWindowFlags_HorizontalScrollbar;
    ...
    else
    {
        ...
        table->InnerWindow->DC.TreeDepth++; // This is designed to always linking HvkGuiTreeNodeFlags_DrawLines linking across a table
    }
    ...
    inner_window->DC.PrevLineSize = inner_window->DC.CurrLineSize = HvkVec2(0.0f, 0.0f);
    ...
    // limits of that rectangle, in order for HvkDrawListSplitter::Merge() to merge the draw commands. However since the overlap
    ...
    const bool pad_outer_x = (flags & HvkGuiTableFlags_NoPadOuterX) ? false : (flags & HvkGuiTableFlags_PadOuterX) ? true : (flags & HvkGuiTableFlags_BordersOuterV) != 0;
    const bool pad_inner_x = (flags & HvkGuiTableFlags_NoPadInnerX) ? false : true;
    const float inner_spacing_for_border = (flags & HvkGuiTableFlags_BordersInnerV) ? TABLE_BORDER_SIZE : 0.0f;
    const float inner_spacing_explicit = (pad_inner_x && (flags & HvkGuiTableFlags_BordersInnerV) == 0) ? g.Style.CellPadding.x : 0.0f;
    const float inner_padding_explicit = (pad_inner_x && (flags & HvkGuiTableFlags_BordersInnerV) != 0) ? g.Style.CellPadding.x : 0.0f;
    table->CellSpacingX1 = inner_spacing_explicit + inner_spacing_for_border;
    table->CellSpacingX2 = inner_spacing_explicit;
    table->CellPaddingX = inner_padding_explicit;

    const float outer_padding_for_border = (flags & HvkGuiTableFlags_BordersOuterV) ? TABLE_BORDER_SIZE : 0.0f;
    ...
    table->LastRowFlags = HvkGuiTableRowFlags_None;
    ...
    // Using opaque colors facilitate overlapping lines of the grid, otherwise we'd need to Hvkprove TableDrawBorders()
    table->BorderColorStrong = GetColorU32(HvkGuiCol_TableBorderStrong);
    table->BorderColorLight = GetColorU32(HvkGuiCol_TableBorderLight);
    ...
    if ((previous_flags & HvkGuiTableFlags_Reorderable) && (flags & HvkGuiTableFlags_Reorderable) == 0)
    ...
    HvkGuiTableColumn* old_columns_to_preserve = NULL;
    ...
    if (table->IsInitializing)
    {
        ...
        for (int n = 0; n < columns_count; n++)
        {
            ...
            else
            {
                ...
                *column = HvkGuiTableColumn();
                column->WidthAuto = width_auto;
                column->IsPreserveWidthAuto = true; // Preserve WidthAuto when reinitializing a live table: not technically necessary but remove a visible flicker
                column->IsEnabled = column->IsUserEnabled = column->IsUserEnabledNextFrame = true;
                column->DisplayOrder = (HvkGuiTableColumnIdx)n;
            }
    ...
    if (table->RefScale != 0.0f && table->RefScale != new_ref_scale_unit)
    {
        ...
        //HvkGui_DEBUG_PRINT("[table] %08X RefScaleUnit %.3f -> %.3f, scaling width by %.3f\n", table->ID, table->RefScaleUnit, new_ref_scale_unit, scale_factor);
        for (int n = 0; n < columns_count; n++)
...
// + 0 (for HvkGuiTable instance, we are pooling allocations in g.Tables[])
...
// + 2 * active_channels_count (for HvkDrawCmd and HvkDrawIdx buffers inside channels)
// Where active_channels_count is variable but often == columns_count or == columns_count + 1, see TableSetupDrawChannels() for details.
// Unused channels don't perform their +2 allocations.
void HvkGui::TableBeginInitMemory(HvkGuiTable* table, int columns_count)
{
    // Allocate single buffer for our arrays
    const int columns_bit_array_size = (int)HvkBitArrayGetStorageSizeInBytes(columns_count);
    HvkSpanAllocator<6> span_allocator;
    span_allocator.Reserve(0, columns_count * sizeof(HvkGuiTableColumn));
    span_allocator.Reserve(1, columns_count * sizeof(HvkGuiTableColumnIdx));
    ...
    table->EnabledMaskByDisplayOrder = (HvkU32*)span_allocator.GetSpanPtrBegin(3);
    table->EnabledMaskByIndex = (HvkU32*)span_allocator.GetSpanPtrBegin(4);
    table->VisibleMaskByIndex = (HvkU32*)span_allocator.GetSpanPtrBegin(5);
}

// Apply queued resizing/reordering/hiding requests
void HvkGui::TableBeginApplyRequests(HvkGuiTable* table)
{
    ...
    if (table->InstanceCurrent == 0)
    {
        ...
        if (table->ReorderColumn != -1 && table->ReorderColumnDir != 0)
        {
            ...
            for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
                table->DisplayOrderToIndex[table->Columns[column_n].DisplayOrder] = (HvkGuiTableColumnIdx)column_n;
    ...
    if (table->IsResetDisplayOrderRequest)
    {
        for (int n = 0; n < table->ColumnsCount; n++)
            table->DisplayOrderToIndex[n] = table->Columns[n].DisplayOrder = (HvkGuiTableColumnIdx)n;
...
static void TableSetupColumnFlags(HvkGuiTable* table, HvkGuiTableColumn* column, HvkGuiTableColumnFlags flags_in)
{
    HvkGuiTableColumnFlags flags = flags_in;

    // Sizing Policy
    if ((flags & HvkGuiTableColumnFlags_WidthMask_) == 0)
    {
        const HvkGuiTableFlags table_sizing_policy = (table->Flags & HvkGuiTableFlags_SizingMask_);
        if (table_sizing_policy == HvkGuiTableFlags_SizingFixedFit || table_sizing_policy == HvkGuiTableFlags_SizingFixedSame)
            flags |= HvkGuiTableColumnFlags_WidthFixed;
        else
            flags |= HvkGuiTableColumnFlags_WidthStretch;
    }
    else
    {
    ...
    // Resize
    if ((table->Flags & HvkGuiTableFlags_Resizable) == 0)
        flags |= HvkGuiTableColumnFlags_NoResize;

    // Sorting
    if ((flags & HvkGuiTableColumnFlags_NoSortAscending) && (flags & HvkGuiTableColumnFlags_NoSortDescending))
        flags |= HvkGuiTableColumnFlags_NoSort;

    // Indentation
    if ((flags & HvkGuiTableColumnFlags_IndentMask_) == 0)
        flags |= (table->Columns.index_from_ptr(column) == 0) ? HvkGuiTableColumnFlags_IndentEnable : HvkGuiTableColumnFlags_IndentDisable;
    ...
    if (table->Flags & HvkGuiTableFlags_Sortable)
    {
        int count = 0, mask = 0, list = 0;
        if ((flags & HvkGuiTableColumnFlags_PreferSortAscending)  != 0 && (flags & HvkGuiTableColumnFlags_NoSortAscending)  == 0) { mask |= 1 << HvkGuiSortDirection_Ascending;  list |= HvkGuiSortDirection_Ascending  << (count << 1); count++; }
        if ((flags & HvkGuiTableColumnFlags_PreferSortDescending) != 0 && (flags & HvkGuiTableColumnFlags_NoSortDescending) == 0) { mask |= 1 << HvkGuiSortDirection_Descending; list |= HvkGuiSortDirection_Descending << (count << 1); count++; }
        if ((flags & HvkGuiTableColumnFlags_PreferSortAscending)  == 0 && (flags & HvkGuiTableColumnFlags_NoSortAscending)  == 0) { mask |= 1 << HvkGuiSortDirection_Ascending;  list |= HvkGuiSortDirection_Ascending  << (count << 1); count++; }
        if ((flags & HvkGuiTableColumnFlags_PreferSortDescending) == 0 && (flags & HvkGuiTableColumnFlags_NoSortDescending) == 0) { mask |= 1 << HvkGuiSortDirection_Descending; list |= HvkGuiSortDirection_Descending << (count << 1); count++; }
        if ((table->Flags & HvkGuiTableFlags_SortTristate) || count == 0) { mask |= 1 << HvkGuiSortDirection_None; count++; }
        column->SortDirectionsAvailList = (HvkU8)list;
        column->SortDirectionsAvailMask = (HvkU8)mask;
        column->SortDirectionsAvailCount = (HvkU8)count;
        HvkGui::TableFixColumnSortDirection(table, column);
    }
}
...
void HvkGui::TableUpdateLayout(HvkGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        {
            TableSetupColumnFlags(table, column, HvkGuiTableColumnFlags_None);
            column->NameOffset = -1;
            column->UserID = 0;
        ...
        if (!(table->Flags & HvkGuiTableFlags_Hideable) || (column->Flags & HvkGuiTableColumnFlags_NoHide))
        ...
        column->IsEnabled = column->IsUserEnabled && (column->Flags & HvkGuiTableColumnFlags_Disabled) == 0;

        if (column->SortOrder != -1 && !column->IsEnabled)
            table->IsSortSpecsDirty = true;
        if (column->SortOrder > 0 && !(table->Flags & HvkGuiTableFlags_SortMulti))
            table->IsSortSpecsDirty = true;

        // Auto-fit unsized columns
        const bool start_auto_fit = (column->Flags & HvkGuiTableColumnFlags_WidthFixed) ? (column->WidthRequest < 0.0f) : (column->StretchWeight < 0.0f);
        if (start_auto_fit)
        ...
        column->PrevEnabledColumn = (HvkGuiTableColumnIdx)prev_visible_column_idx;
        column->NextEnabledColumn = -1;
        if (prev_visible_column_idx != -1)
            table->Columns[prev_visible_column_idx].NextEnabledColumn = (HvkGuiTableColumnIdx)column_n;
        else
            table->LeftMostEnabledColumn = (HvkGuiTableColumnIdx)column_n;
        ...
        const bool column_is_resizable = (column->Flags & HvkGuiTableColumnFlags_NoResize) == 0;
        if (column_is_resizable)
            has_resizable = true;
        if ((column->Flags & HvkGuiTableColumnFlags_WidthFixed) && column->InitStretchWeightOrWidth > 0.0f && !column_is_resizable)
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        const bool column_is_resizable = (column->Flags & HvkGuiTableColumnFlags_NoResize) == 0;
        if (column->Flags & HvkGuiTableColumnFlags_WidthFixed)
        {
            // Apply same widths policy
            float width_auto = column->WidthAuto;
            if (table_sizing_policy == HvkGuiTableFlags_SizingFixedSame && (column->AutoFitQueue != 0x00 || !column_is_resizable))
            ...
            else if ((column->Flags & HvkGuiTableColumnFlags_WidthFixed) && !column_is_resizable && column->IsRequestOutput)
                column->WidthRequest = width_auto;
        ...
        else
        {
            ...
            if (column->AutoFitQueue != 0x00 || column->StretchWeight < 0.0f || !column_is_resizable)
            {
                ...
                else if (table_sizing_policy == HvkGuiTableFlags_SizingStretchProp)
            ...
            if (table->LeftMostStretchedColumn == -1 || table->Columns[table->LeftMostStretchedColumn].DisplayOrder > column->DisplayOrder)
                table->LeftMostStretchedColumn = (HvkGuiTableColumnIdx)column_n;
            if (table->RightMostStretchedColumn == -1 || table->Columns[table->RightMostStretchedColumn].DisplayOrder < column->DisplayOrder)
                table->RightMostStretchedColumn = (HvkGuiTableColumnIdx)column_n;
        }
        column->IsPreserveWidthAuto = false;
        sum_width_requests += table->CellPaddingX * 2.0f;
    }
    table->ColumnsEnabledFixedCount = (HvkGuiTableColumnIdx)count_fixed;
    ...
    // - In principle we'd like to use the equivalent of IsItemHovered(HvkGuiHoveredFlags_AllowWhenBlockedByActiveItem),
    ...
    if (table->AngledHeadersHeight > 0.0f)
        if (g.IO.MousePos.y >= table->OuterRect.Min.y && g.IO.MousePos.y <= table->OuterRect.Min.y + table->AngledHeadersHeight)
            mouse_skewed_x += HvkTrunc((table->OuterRect.Min.y + table->AngledHeadersHeight - g.IO.MousePos.y) * table->AngledHeadersSlope);
    ...
    HvkRect host_clip_rect = table->InnerClipRect;
    //host_clip_rect.Max.x += table->CellPaddingX + table->CellSpacingX2;
    HvkBitArrayClearAllBits(table->VisibleMaskByIndex, table->ColumnsCount);
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        const int column_n = table->DisplayOrderToIndex[order_n];
        HvkGuiTableColumn* column = &table->Columns[column_n];
        ...
        column->NavLayerCurrent = (HvkS8)(table->FreezeRowsCount > 0 ? HvkGuiNavLayer_Menu : (HvkGuiNavLayer)table->NavLayer);
        ...
        column->ItemWidth = HvkTrunc(column->WidthGiven * 0.65f);
        ...
        if (is_visible)
            HvkBitArraySetBit(table->VisibleMaskByIndex, column_n);
        ...
        // Update status flags
        column->Flags |= HvkGuiTableColumnFlags_IsEnabled;
        if (is_visible)
            column->Flags |= HvkGuiTableColumnFlags_IsVisible;
        if (column->SortOrder != -1)
            column->Flags |= HvkGuiTableColumnFlags_IsSorted;

        // Detect hovered column
        if (is_hovering_table && mouse_skewed_x >= column->ClipRect.Min.x && mouse_skewed_x < column->ClipRect.Max.x)
        {
            column->Flags |= HvkGuiTableColumnFlags_IsHovered;
            table->HoveredColumnBody = (HvkGuiTableColumnIdx)column_n;
        }
        ...
        if (visible_n < table->FreezeColumnsCount)
            host_clip_rect.Min.x = HvkClamp(column->MaxX + TABLE_BORDER_SIZE, host_clip_rect.Min.x, host_clip_rect.Max.x);
    ...
    if (has_resizable == false && (table->Flags & HvkGuiTableFlags_Resizable))
        table->Flags &= ~HvkGuiTableFlags_Resizable;
    ...
    if (table->RightMostStretchedColumn != -1)
        table->Flags &= ~HvkGuiTableFlags_NoHostExtendX;
    ...
    if (table->Flags & HvkGuiTableFlags_Resizable)
    ...
    else if ((table->Flags & HvkGuiTableFlags_HighlightHoveredColumn) && table->HoveredColumnBody != -1 && table->HoveredColumnBody != table->ColumnsCount && table->HoveredColumnBorder == -1)
    ...
    //   e.g. TableDrawDefaultContextMenu(table, table->Flags & ~HvkGuiTableFlags_Hideable) will display everything EXCEPT columns visibility options.
    ...
    if (table->IsSortSpecsDirty && (table->Flags & HvkGuiTableFlags_Sortable))
    ...
    // Initial state
    HvkGuiWindow* inner_window = table->InnerWindow;
    if (table->Flags & HvkGuiTableFlags_NoClip)
...
void HvkGui::TableUpdateBorders(HvkGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        const int column_n = table->DisplayOrderToIndex[order_n];
        HvkGuiTableColumn* column = &table->Columns[column_n];
        if (column->Flags & (HvkGuiTableColumnFlags_NoResize | HvkGuiTableColumnFlags_NoDirectResize_))
            continue;

        // HvkGuiTableFlags_NoBordersInBodyUntilResize will be honored in TableDrawBorders()
        const float border_y2_hit = (table->Flags & HvkGuiTableFlags_NoBordersInBody) ? hit_y2_head : hit_y2_body;
        if ((table->Flags & HvkGuiTableFlags_NoBordersInBody) && table->IsUsingHeaders == false)
            continue;
        ...
        if (held)
        {
            ...
            table->ResizedColumn = (HvkGuiTableColumnIdx)column_n;
            table->InstanceInteracted = table->InstanceCurrent;
        }
        if ((hovered && g.HoveredIdTimer > TABLE_RESIZE_SEPARATOR_FEEDBACK_TIMER) || held)
        {
            table->HoveredColumnBorder = (HvkGuiTableColumnIdx)column_n;
            SetMouseCursor(HvkGuiMouseCursor_ResizeEW);
        }
    }
}

void    HvkGui::EndTable()
{
    HvkGuiContext& g = *GHvkGui;
    ...
    const HvkGuiTableFlags flags = table->Flags;
    HvkGuiWindow* inner_window = table->InnerWindow;
    ...
    if (flags & HvkGuiTableFlags_ContextMenuInBody)
        if (table->HoveredColumnBody != -1 && !IsAnyItemHovered() && IsMouseReleased(HvkGuiMouseButton_Right))
    ...
    // but since the later is likely to be Hvkpossible to do we'd rather update both axes together.
    if (table->Flags & HvkGuiTableFlags_ScrollX)
    {
    ...
    // Pop clipping rect
    if (!(flags & HvkGuiTableFlags_NoClip))
        inner_window->DrawList->PopClipRect();
    inner_window->ClipRect = inner_window->DrawList->_ClipRectStack.back();

    // Draw borders
    if ((flags & HvkGuiTableFlags_Borders) != 0)
        TableDrawBorders(table);

#if 0
    // Strip out dummy channel draw calls
    // We have no way to prevent user submitting direct HvkDrawList calls into a hidden column (but HvkGui:: calls will be clipped out)
    // Pros: remove draw calls which will have no effect. since they'll have zero-size cliprect they may be early out anyway.
    // Cons: making it harder for users watching metrics/debugger to spot the wasted vertices.
    if (table->DummyDrawChannel != (HvkGuiTableColumnIdx)-1)
    {
        HvkDrawChannel* dummy_channel = &table->DrawSplitter._Channels[table->DummyDrawChannel];
    ...
    if ((table->Flags & HvkGuiTableFlags_NoClip) == 0)
    ...
    else if (table->LastResizedColumn != -1 && table->ResizedColumn == -1 && inner_window->ScrollbarX && table->InstanceInteracted == table->InstanceCurrent)
    {
        ...
        HvkGuiTableColumn* column = &table->Columns[table->LastResizedColumn];
    ...
    {
        HvkGuiTableColumn* column = &table->Columns[table->ResizedColumn];
        const float new_x2 = (g.IO.MousePos.x - g.ActiveIdClickOffset.x + HvkTrunc(TABLE_RESIZE_SEPARATOR_HALF_THICKNESS * g.CurrentDpiScale));
        const float new_width = HvkTrunc(new_x2 - column->MinX - table->CellSpacingX1 - table->CellPaddingX * 2.0f);
    ...
    const HvkVec2 backup_outer_max_pos = outer_window->DC.CursorMaxPos;
    ...
    else if (temp_data->UserOuterSize.x <= 0.0f)
    {
        ...
        // - Checking for HvkGuiTableFlags_ScrollX/ScrollY flag makes us a frame ahead when disabling those flags.
...
static void TableInitColumnDefaults(HvkGuiTable* table, HvkGuiTableColumn* column, HvkGuiTableColumnFlags init_mask)
{
    HvkGuiTableColumnFlags flags = column->Flags;
    if (init_mask & HvkGuiTableFlags_Resizable)
    {
        float init_width_or_weight = column->InitStretchWeightOrWidth;
        column->WidthRequest = ((flags & HvkGuiTableColumnFlags_WidthFixed) && init_width_or_weight > 0.0f) ? init_width_or_weight : -1.0f;
        column->StretchWeight = (init_width_or_weight > 0.0f && (flags & HvkGuiTableColumnFlags_WidthStretch)) ? init_width_or_weight : -1.0f;
        if (init_width_or_weight > 0.0f) // Disable auto-fit if an explicit width/weight has been specified
            column->AutoFitQueue = 0x00;
    }
    if (init_mask & HvkGuiTableFlags_Reorderable)
        column->DisplayOrder = (HvkGuiTableColumnIdx)table->Columns.index_from_ptr(column);
    if (init_mask & HvkGuiTableFlags_Hideable)
        column->IsUserEnabled = column->IsUserEnabledNextFrame = (flags & HvkGuiTableColumnFlags_DefaultHide) ? 0 : 1;
    if (init_mask & HvkGuiTableFlags_Sortable)
    {
        // Multiple columns using _DefaultSort will be reassigned unique SortOrder values when building the sort specs.
        column->SortOrder = (flags & HvkGuiTableColumnFlags_DefaultSort) ? 0 : -1;
        column->SortDirection = (flags & HvkGuiTableColumnFlags_DefaultSort) ? ((flags & HvkGuiTableColumnFlags_PreferSortDescending) ? (HvkS8)HvkGuiSortDirection_Descending : (HvkU8)(HvkGuiSortDirection_Ascending)) : (HvkS8)HvkGuiSortDirection_None;
    }
}

// See "COLUMNS SIZING POLICIES" comments at the top of this file
// If (init_width_or_weight <= 0.0f) it is ignored
void HvkGui::TableSetupColumn(const char* label, HvkGuiTableColumnFlags flags, float init_width_or_weight, HvkGuiID user_id)
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTable* table = g.CurrentTable;
    if (table == NULL)
    ...
    HvkGuiTableColumn* column = &table->Columns[table->DeclColumnsCount];
    ...
    if ((flags & HvkGuiTableColumnFlags_WidthMask_) == 0 && init_width_or_weight > 0.0f)
        if ((table->Flags & HvkGuiTableFlags_SizingMask_) == HvkGuiTableFlags_SizingFixedFit || (table->Flags & HvkGuiTableFlags_SizingMask_) == HvkGuiTableFlags_SizingFixedSame)
            flags |= HvkGuiTableColumnFlags_WidthFixed;
    if (flags & HvkGuiTableColumnFlags_AngledHeader)
    {
        flags |= HvkGuiTableColumnFlags_NoHeaderLabel;
    ...
    if (table->IsInitializing)
    {
        HvkGuiTableFlags init_flags = ~table->SettingsLoadedFlags;
        if (column->WidthRequest < 0.0f && column->StretchWeight < 0.0f)
            init_flags |= HvkGuiTableFlags_Resizable;
    ...
    {
        column->NameOffset = (HvkS16)table->ColumnsNames.size();
        table->ColumnsNames.append(label, label + HvkStrlen(label) + 1);
    }
}

// [Public]
void HvkGui::TableSetupScrollFreeze(int columns, int rows)
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTable* table = g.CurrentTable;
    if (table == NULL)
    ...
    table->IsUnfrozenRows = (table->FreezeRowsCount == 0); // Make sure this is set before TableUpdateLayout() so HvkGuiListClipper can benefit from it.b
    ...
    for (int column_n = 0; column_n < table->FreezeColumnsRequest; column_n++)
    {
        ...
        {
            HvkSwap(table->Columns[table->DisplayOrderToIndex[order_n]].DisplayOrder, table->Columns[table->DisplayOrderToIndex[column_n]].DisplayOrder);
            HvkSwap(table->DisplayOrderToIndex[order_n], table->DisplayOrderToIndex[column_n]);
        }
    }
}
...
int HvkGui::TableGetColumnCount()
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTable* table = g.CurrentTable;
...
const char* HvkGui::TableGetColumnName(int column_n)
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTable* table = g.CurrentTable;
    if (!table)
...
const char* HvkGui::TableGetColumnName(const HvkGuiTable* table, int column_n)
{
    if (table->IsLayoutLocked == false && column_n >= table->DeclColumnsCount)
        return ""; // NameOffset is invalid at this point
    const HvkGuiTableColumn* column = &table->Columns[column_n];
...
// Note that end-user can use the context menu to change this themselves (right-click in headers, or right-click in columns body with HvkGuiTableFlags_ContextMenuInBody)
// - Require table to have the HvkGuiTableFlags_Hideable flag because we are manipulating user accessible state.
// - Request will be applied during next layout, which happens on the first call to TableNextRow() after BeginTable().
// - For the getter you can test (TableGetColumnFlags() & HvkGuiTableColumnFlags_IsEnabled) != 0.
// - Alternative: the HvkGuiTableColumnFlags_Disabled is an overriding/master disable flag which will also hide the column from context menu.
void HvkGui::TableSetColumnEnabled(int column_n, bool enabled)
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTable* table = g.CurrentTable;
    if (table == NULL)
...
HvkGuiTableColumnFlags HvkGui::TableGetColumnFlags(int column_n)
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTable* table = g.CurrentTable;
    if (!table)
        return HvkGuiTableColumnFlags_None;
    if (column_n < 0)
        column_n = table->CurrentColumn;
    if (column_n == table->ColumnsCount)
        return (table->HoveredColumnBody == column_n) ? HvkGuiTableColumnFlags_IsHovered : HvkGuiTableColumnFlags_None;
    return table->Columns[column_n].Flags;
}

// Return the cell rectangle based on currently known height.
// - Hvkportant: we generally don't know our row height until the end of the row, so Max.y will be incorrect in many situations.
...
// - Hvkportant: if HvkGuiTableFlags_PadOuterX is set but HvkGuiTableFlags_PadInnerX is not set, the outer-most left and right
//   columns report a small offset so their CellBgRect can extend up to the outer border.
//   FIXME: But the rendering code in TableEndRow() nullifies that with clamping required for scrolling.
HvkRect HvkGui::TableGetCellBgRect(const HvkGuiTable* table, int column_n)
{
    const HvkGuiTableColumn* column = &table->Columns[column_n];
...
int HvkGui::TableGetHoveredColumn()
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTable* table = g.CurrentTable;
    if (!table)
...
// *HvkPORTANT* Unlike TableGetHoveredColumn(), this has a one frame latency in updating the value.
// This difference with is the reason why this is not public yet.
int HvkGui::TableGetHoveredRow()
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTable* table = g.CurrentTable;
    if (!table)
...
void HvkGui::TableSetBgColor(HvkGuiTableBgTarget target, HvkU32 color, int column_n)
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTable* table = g.CurrentTable;
    ...
    case HvkGuiTableBgTarget_CellBg:
    {
        ...
        HvkGuiTableCellData* cell_data = &table->RowCellData[table->RowCellDataCurrent];
        cell_data->BgColor = color;
        cell_data->Column = (HvkGuiTableColumnIdx)column_n;
        break;
    }
    case HvkGuiTableBgTarget_RowBg0:
    case HvkGuiTableBgTarget_RowBg1:
    {
        if (table->RowPosY1 > table->InnerClipRect.Max.y) // Discard
            return;
...
void HvkGui::TableNextRow(HvkGuiTableRowFlags row_flags, float row_min_height)
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTable* table = g.CurrentTable;
...
void HvkGui::TableBeginRow(HvkGuiTable* table)
{
    ...
    window->DC.CursorPosPrevLine = HvkVec2(window->DC.CursorPos.x, window->DC.CursorPos.y + table->RowCellPaddingY); // This allows users to call SameLine() to share LineSize between columns.
    window->DC.PrevLineSize = window->DC.CurrLineSize = HvkVec2(0.0f, 0.0f); // This allows users to call SameLine() to share LineSize between columns, and to call it from first column too.
    window->DC.IsSameLine = window->DC.IsSetPos = false;
    window->DC.CursorMaxPos.y = next_y1;

    // Making the header BG color non-transparent will allow us to overlay it multiple times when handling smooth dragging.
    if (table->RowFlags & HvkGuiTableRowFlags_Headers)
    {
        TableSetBgColor(HvkGuiTableBgTarget_RowBg0, GetColorU32(HvkGuiCol_TableHeaderBg));
...
void HvkGui::TableEndRow(HvkGuiTable* table)
{
    HvkGuiContext& g = *GHvkGui;
    ...
    HvkGuiTableInstanceData* table_instance = TableGetInstanceData(table, table->InstanceCurrent);
    if ((table->RowFlags & HvkGuiTableRowFlags_Headers) && (table->CurrentRow == 0 || (table->LastRowFlags & HvkGuiTableRowFlags_Headers)))
    ...
    if (is_visible)
    {
        ...
        HvkU32 top_border_col = 0;
        const float border_size = TABLE_BORDER_SIZE;
        if (table->CurrentRow > 0 && (table->Flags & HvkGuiTableFlags_BordersInnerH))
            top_border_col = (table->LastRowFlags & HvkGuiTableRowFlags_Headers) ? table->BorderColorStrong : table->BorderColorLight;
        ...
        if (bg_col0 || bg_col1)
        {
            HvkRect row_rect(table->WorkRect.Min.x, bg_y1, table->WorkRect.Max.x, bg_y2);
        ...
        {
            HvkGuiTableCellData* cell_data_end = &table->RowCellData[table->RowCellDataCurrent];
            for (HvkGuiTableCellData* cell_data = &table->RowCellData[0]; cell_data <= cell_data_end; cell_data++)
            {
        ...
        if (top_border_col && bg_y1 >= table->BgClipRect.Min.y && bg_y1 < table->BgClipRect.Max.y)
            window->DrawList->AddLine(HvkVec2(table->BorderX1, bg_y1), HvkVec2(table->BorderX2, bg_y1), top_border_col, border_size);

        // Draw bottom border at the row unfreezing mark (always strong)
        if (draw_strong_bottom_border && bg_y2 >= table->BgClipRect.Min.y && bg_y2 < table->BgClipRect.Max.y)
            window->DrawList->AddLine(HvkVec2(table->BorderX1, bg_y2), HvkVec2(table->BorderX2, bg_y2), table->BorderColorStrong, border_size);
    }
...
int HvkGui::TableGetColumnIndex()
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTable* table = g.CurrentTable;
    if (!table)
...
bool HvkGui::TableSetColumnIndex(int column_n)
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTable* table = g.CurrentTable;
    if (!table)
...
void HvkGui::TableBeginCell(HvkGuiTable* table, int column_n)
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTableColumn* column = &table->Columns[column_n];
    HvkGuiWindow* window = table->InnerWindow;
    ...
    if (column->Flags & HvkGuiTableColumnFlags_IndentEnable)
    ...
    window->DC.NavLayerCurrent = (HvkGuiNavLayer)column->NavLayerCurrent;
...
void HvkGui::TableEndCell(HvkGuiTable* table)
{
    HvkGuiTableColumn* column = &table->Columns[table->CurrentColumn];
    HvkGuiWindow* window = table->InnerWindow;
...
float HvkGui::TableCalcMaxColumnWidth(const HvkGuiTable* table, int column_n)
{
    const HvkGuiTableColumn* column = &table->Columns[column_n];
    ...
    else if ((table->Flags & HvkGuiTableFlags_NoKeepColumnsVisible) == 0)
    {
...
void HvkGui::TableSetColumnWidth(int column_n, float width)
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTable* table = g.CurrentTable;
    ...
    //HvkGui_DEBUG_PRINT("TableSetColumnWidth(%d, %.1f->%.1f)\n", column_0_idx, column_0->WidthGiven, column_0_width);
    HvkGuiTableColumn* column_1 = (column_0->NextEnabledColumn != -1) ? &table->Columns[column_0->NextEnabledColumn] : NULL;
    ...
    if (column_0->Flags & HvkGuiTableColumnFlags_WidthFixed)
...
void HvkGui::TableSetColumnWidthAutoSingle(HvkGuiTable* table, int column_n)
{
    ...
    table->AutoFitSingleColumn = (HvkGuiTableColumnIdx)column_n;
}

void HvkGui::TableSetColumnWidthAutoAll(HvkGuiTable* table)
{
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        HvkGuiTableColumn* column = &table->Columns[column_n];
        if (!column->IsEnabled && !(column->Flags & HvkGuiTableColumnFlags_WidthStretch)) // Cannot reset weight of hidden stretch column
            continue;
        column->CannotSkipItemsQueue = (1 << 0);
...
void HvkGui::TablePushBackgroundChannel()
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiWindow* window = g.CurrentWindow;
    HvkGuiTable* table = g.CurrentTable;
...
void HvkGui::TablePopBackgroundChannel()
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiWindow* window = g.CurrentWindow;
    HvkGuiTable* table = g.CurrentTable;
...
void HvkGui::TablePushColumnChannel(int column_n)
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTable* table = g.CurrentTable;
...
void HvkGui::TablePopColumnChannel()
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTable* table = g.CurrentTable;

    // Optimization: avoid PopClipRect() + SetCurrentChannel()
    if ((table->Flags & HvkGuiTableFlags_NoClip) || (table->CurrentColumn == -1)) // Calling TreePop() after TableNextRow() is supported.
        return;
    HvkGuiWindow* window = g.CurrentWindow;
    const HvkGuiTableColumn* column = &table->Columns[table->CurrentColumn];
    SetWindowClipRectBeforeSetChannel(window, column->ClipRect);
    table->DrawSplitter->SetCurrentChannel(window->DrawList, column->DrawChannelCurrent);
}
...
void HvkGui::TableSetupDrawChannels(HvkGuiTable* table)
{
    const int freeze_row_multiplier = (table->FreezeRowsCount > 0) ? 2 : 1;
    const int channels_for_row = (table->Flags & HvkGuiTableFlags_NoClip) ? 1 : table->ColumnsEnabledCount;
    const int channels_for_bg = 1 + 1 * freeze_row_multiplier;
    const int channels_for_dummy = (table->ColumnsEnabledCount < table->ColumnsCount || (memcmp(table->VisibleMaskByIndex, table->EnabledMaskByIndex, HvkBitArrayGetStorageSizeInBytes(table->ColumnsCount)) != 0)) ? +1 : 0;
    const int channels_total = channels_for_bg + (channels_for_row * freeze_row_multiplier) + channels_for_dummy;
    table->DrawSplitter->Split(table->InnerWindow->DrawList, channels_total);
    table->DummyDrawChannel = (HvkGuiTableDrawChannelIdx)((channels_for_dummy > 0) ? channels_total - 1 : -1);
    table->Bg2DrawChannelCurrent = TABLE_DRAW_CHANNEL_BG2_FROZEN;
    table->Bg2DrawChannelUnfrozen = (HvkGuiTableDrawChannelIdx)((table->FreezeRowsCount > 0) ? 2 + channels_for_row : TABLE_DRAW_CHANNEL_BG2_FROZEN);
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        {
            column->DrawChannelFrozen = (HvkGuiTableDrawChannelIdx)(draw_channel_current);
            column->DrawChannelUnfrozen = (HvkGuiTableDrawChannelIdx)(draw_channel_current + (table->FreezeRowsCount > 0 ? channels_for_row + 1 : 0));
            if (!(table->Flags & HvkGuiTableFlags_NoClip))
...
// At the end of the operation our 1-4 groups will each have a HvkDrawCmd using the same ClipRect.
// This function assume that each column are pointing to a distinct draw channel,
// otherwise merge_group->ChannelsCount will not match set bit count of merge_group->ChannelsMask.
//
// Column channels will not be merged into one of the 1-4 groups in the following cases:
// - The contents stray off its clipping rectangle (we only compare the MaxX value, not the MinX value).
//   Direct HvkDrawList calls won't be taken into account by default, if you use them make sure the HvkGui:: bounds
//   matches, by e.g. calling SetCursorScreenPos().
// - The channel uses more than one draw command itself. We drop all our attempt at merging stuff here..
//   we could do better but it's going to be rare and probably not worth the hassle.
// Columns for which the draw channel(s) haven't been merged with other will use their own HvkDrawCmd.
//
// This function is particularly tricky to understand.. take a breath.
void HvkGui::TableMergeDrawChannels(HvkGuiTable* table)
{
    HvkGuiContext& g = *GHvkGui;
    HvkDrawListSplitter* splitter = table->DrawSplitter;
    ...
    struct MergeGroup
    {
        HvkRect          ClipRect;
        int             ChannelsCount = 0;
        HvkBitArrayPtr   ChannelsMask = NULL;
    };
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        for (int merge_group_sub_n = 0; merge_group_sub_n < merge_group_sub_count; merge_group_sub_n++)
        {
            ...
            HvkDrawChannel* src_channel = &splitter->_Channels[channel_no];
            ...
            if (!(column->Flags & HvkGuiTableColumnFlags_NoClip))
            {
                float content_max_x;
                if (!has_freeze_v)
            ...
            HvkBitArraySetBit(merge_group->ChannelsMask, channel_no);
        ...
        column->DrawChannelCurrent = (HvkGuiTableDrawChannelIdx)-1;
    }
    ...
    if (merge_group_mask != 0)
    {
        ...
        for (int merge_group_n = 0; merge_group_n < Hvk_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            ...
            {
                MergeGroup* merge_group = &merge_groups[merge_group_n];
                HvkRect merge_clip_rect = merge_group->ClipRect;
            ...
            if (merge_group_n == 1 && has_freeze_v)
                memcpy(dst_tmp++, &splitter->_Channels[table->Bg2DrawChannelUnfrozen], sizeof(HvkDrawChannel));
        }
...
static HvkU32 TableGetColumnBorderCol(HvkGuiTable* table, int order_n, int column_n)
{
    const bool is_hovered = (table->HoveredColumnBorder == column_n);
    const bool is_resized = (table->ResizedColumn == column_n) && (table->InstanceInteracted == table->InstanceCurrent);
    const bool is_frozen_separator = (table->FreezeColumnsCount == order_n + 1);
    if (is_resized || is_hovered)
        return HvkGui::GetColorU32(is_resized ? HvkGuiCol_SeparatorActive : HvkGuiCol_SeparatorHovered);
    if (is_frozen_separator || (table->Flags & (HvkGuiTableFlags_NoBordersInBody | HvkGuiTableFlags_NoBordersInBodyUntilResize)))
        return table->BorderColorStrong;
    return table->BorderColorLight;
}

// FIXME-TABLE: This is a mess, need to redesign how we render borders (as some are also done in TableEndRow)
void HvkGui::TableDrawBorders(HvkGuiTable* table)
{
    HvkGuiWindow* inner_window = table->InnerWindow;
    if (!table->OuterWindow->ClipRect.Overlaps(table->OuterRect))
        return;

    HvkDrawList* inner_drawlist = inner_window->DrawList;
    ...
    if (table->Flags & HvkGuiTableFlags_BordersInnerV)
    {
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            const bool is_hovered = (table->HoveredColumnBorder == column_n);
            const bool is_resized = (table->ResizedColumn == column_n) && (table->InstanceInteracted == table->InstanceCurrent);
            const bool is_resizable = (column->Flags & (HvkGuiTableColumnFlags_NoResize | HvkGuiTableColumnFlags_NoDirectResize_)) == 0;
            const bool is_frozen_separator = (table->FreezeColumnsCount == order_n + 1);
            ...
            if (column->NextEnabledColumn == -1 && !is_resizable)
                if ((table->Flags & HvkGuiTableFlags_SizingMask_) != HvkGuiTableFlags_SizingFixedSame || (table->Flags & HvkGuiTableFlags_NoHostExtendX))
                    continue;
            ...
            else if ((table->Flags & HvkGuiTableFlags_NoBordersInBodyUntilResize) != 0 && (is_hovered || is_resized))
                draw_y2 = draw_y2_body;
            else if ((table->Flags & (HvkGuiTableFlags_NoBordersInBodyUntilResize | HvkGuiTableFlags_NoBordersInBody)) == 0)
                draw_y2 = draw_y2_body;
            if (draw_y2 > draw_y1)
                inner_drawlist->AddLine(HvkVec2(column->MaxX, draw_y1), HvkVec2(column->MaxX, draw_y2), TableGetColumnBorderCol(table, order_n, column_n), border_size);
        }
    }

    // Draw outer border
    // FIXME: could use AddRect or explicit VLine/HLine helper?
    if (table->Flags & HvkGuiTableFlags_BordersOuter)
    {
        ...
        const HvkRect outer_border = table->OuterRect;
        const HvkU32 outer_col = table->BorderColorStrong;
        if ((table->Flags & HvkGuiTableFlags_BordersOuter) == HvkGuiTableFlags_BordersOuter)
        {
            inner_drawlist->AddRect(outer_border.Min, outer_border.Max, outer_col, 0.0f, 0, border_size);
        }
        else if (table->Flags & HvkGuiTableFlags_BordersOuterV)
        {
            inner_drawlist->AddLine(outer_border.Min, HvkVec2(outer_border.Min.x, outer_border.Max.y), outer_col, border_size);
            inner_drawlist->AddLine(HvkVec2(outer_border.Max.x, outer_border.Min.y), outer_border.Max, outer_col, border_size);
        }
        else if (table->Flags & HvkGuiTableFlags_BordersOuterH)
        {
            inner_drawlist->AddLine(outer_border.Min, HvkVec2(outer_border.Max.x, outer_border.Min.y), outer_col, border_size);
            inner_drawlist->AddLine(HvkVec2(outer_border.Min.x, outer_border.Max.y), outer_border.Max, outer_col, border_size);
        }
    }
    if ((table->Flags & HvkGuiTableFlags_BordersInnerH) && table->RowPosY2 < table->OuterRect.Max.y)
    {
        // Draw bottom-most row border between it is above outer border.
        const float border_y = table->RowPosY2;
        if (border_y >= table->BgClipRect.Min.y && border_y < table->BgClipRect.Max.y)
            inner_drawlist->AddLine(HvkVec2(table->BorderX1, border_y), HvkVec2(table->BorderX2, border_y), table->BorderColorLight, border_size);
    }
...
// Return NULL if no sort specs (most often when HvkGuiTableFlags_Sortable is not set)
// When 'sort_specs->SpecsDirty == true' you should sort your data. It will be true when sorting specs have
// changed since last call, or the first time. Make sure to set 'SpecsDirty = false' after sorting,
// else you may wastefully sort your data every frame!
// Lifetime: don't hold on this pointer over multiple frames or past any subsequent call to BeginTable()!
HvkGuiTableSortSpecs* HvkGui::TableGetSortSpecs()
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTable* table = g.CurrentTable;
    if (table == NULL || !(table->Flags & HvkGuiTableFlags_Sortable))
        return NULL;
...
void HvkGui::TableFixColumnSortDirection(HvkGuiTable* table, HvkGuiTableColumn* column)
{
    if (column->SortOrder == -1 || (column->SortDirectionsAvailMask & (1 << column->SortDirection)) != 0)
        return;
    column->SortDirection = (HvkU8)TableGetColumnAvailSortDirection(column, 0);
    table->IsSortSpecsDirty = true;
}
...
void HvkGui::TableSetColumnSortDirection(int column_n, HvkGuiSortDirection sort_direction, bool append_to_sort_specs)
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTable* table = g.CurrentTable;
    ...
    if (column->SortDirection == HvkGuiSortDirection_None)
        column->SortOrder = -1;
    ...
    for (int other_column_n = 0; other_column_n < table->ColumnsCount; other_column_n++)
    {
        HvkGuiTableColumn* other_column = &table->Columns[other_column_n];
...
void HvkGui::TableSortSpecsSanitize(HvkGuiTable* table)
{
    ...
    const bool need_fix_single_sort_order = (sort_order_count > 1) && !(table->Flags & HvkGuiTableFlags_SortMulti);
    if (need_fix_linearize || need_fix_single_sort_order)
    {
        HvkU64 fixed_mask = 0x00;
        for (int sort_n = 0; sort_n < sort_order_count; sort_n++)
        {
            ...
            // Fix: Make sure only one column has a SortOrder if HvkGuiTableFlags_MultiSortable is not set.
    ...
    // Fallback default sort order (if no column with the HvkGuiTableColumnFlags_DefaultSort flag)
    if (sort_order_count == 0 && !(table->Flags & HvkGuiTableFlags_SortTristate))
        for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
        {
            HvkGuiTableColumn* column = &table->Columns[column_n];
            if (column->IsEnabled && !(column->Flags & HvkGuiTableColumnFlags_NoSort))
            {
                sort_order_count = 1;
                column->SortOrder = 0;
                column->SortDirection = (HvkU8)TableGetColumnAvailSortDirection(column, 0);
    ...
    table->SortSpecsCount = (HvkGuiTableColumnIdx)sort_order_count;
}

void HvkGui::TableSortSpecsBuild(HvkGuiTable* table)
{
    ...
    // Write output
    // May be able to move all SortSpecs data from table (48 bytes) to HvkGuiTableTempData if we decide to write it back on every BeginTable()
    HvkGuiTableColumnSortSpecs* sort_specs = (table->SortSpecsCount == 0) ? NULL : (table->SortSpecsCount == 1) ? &table->SortSpecsSingle : table->SortSpecsMulti.Data;
    if (dirty && sort_specs != NULL)
        for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
        {
            ...
            sort_spec->SortOrder = (HvkGuiTableColumnIdx)column->SortOrder;
            sort_spec->SortDirection = (HvkGuiSortDirection)column->SortDirection;
        }
...
float HvkGui::TableGetHeaderRowHeight()
{
...
float HvkGui::TableGetHeaderAngledMaxLabelWidth()
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTable* table = g.CurrentTable;
    float width = 0.0f;
...
void HvkGui::TableHeadersRow()
{
    ...
    TableNextRow(HvkGuiTableRowFlags_Headers, row_height);
    ...
    for (int column_n = 0; column_n < columns_count; column_n++)
    {
        ...
        const char* name = (TableGetColumnFlags(column_n) & HvkGuiTableColumnFlags_NoHeaderLabel) ? "" : TableGetColumnName(column_n);
        PushID(column_n);
        TableHeader(name);
        PopID();
    }
    ...
    HvkVec2 mouse_pos = HvkGui::GetMousePos();
...
void HvkGui::TableHeader(const char* label)
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
    ...
    HvkVec2 label_size = CalcTextSize(label, label_end, true);
    HvkVec2 label_pos = window->DC.CursorPos;
    ...
    {
        w_arrow = HvkTrunc(g.FontSize * ARROW_SCALE + g.Style.FramePadding.x);
        if (column->SortOrder != -1)
            sort_arrow = true;
    ...
    bool hovered, held;
    bool pressed = ButtonBehavior(bb, id, &hovered, &held, HvkGuiButtonFlags_AllowOverlap);
    if (held || hovered || highlight)
    {
        const HvkU32 col = GetColorU32(held ? HvkGuiCol_HeaderActive : hovered ? HvkGuiCol_HeaderHovered : HvkGuiCol_Header);
        //RenderFrame(bb.Min, bb.Max, col, false, 0.0f);
        TableSetBgColor(HvkGuiTableBgTarget_CellBg, col, table->CurrentColumn);
    }
    else
    {
        // Submit single cell bg color in the case we didn't submit a full header row
        if ((table->RowFlags & HvkGuiTableRowFlags_Headers) == 0)
            TableSetBgColor(HvkGuiTableBgTarget_CellBg, GetColorU32(HvkGuiCol_TableHeaderBg), table->CurrentColumn);
    }
    RenderNavCursor(bb, id, HvkGuiNavRenderCursorFlags_Compact | HvkGuiNavRenderCursorFlags_NoRounding);
    if (held)
        table->HeldHeaderColumn = (HvkGuiTableColumnIdx)column_n;
    window->DC.CursorPos.y -= g.Style.ItemSpacing.y * 0.5f;

    // Drag and drop to re-order columns.
    // FIXME-TABLE: Scroll request while reordering a column and it lands out of the scrolling zone.
    if (held && (table->Flags & HvkGuiTableFlags_Reorderable) && IsMouseDragging(0) && !g.DragDropActive)
    {
        // While moving a column it will jump on the other side of the mouse, so we also test for MouseDelta.x
        table->ReorderColumn = (HvkGuiTableColumnIdx)column_n;
        table->InstanceInteracted = table->InstanceCurrent;

        // We don't reorder: through the frozen<>unfrozen line, or through a column that is marked with HvkGuiTableColumnFlags_NoReorder.
        if (g.IO.MouseDelta.x < 0.0f && g.IO.MousePos.x < cell_r.Min.x)
            if (HvkGuiTableColumn* prev_column = (column->PrevEnabledColumn != -1) ? &table->Columns[column->PrevEnabledColumn] : NULL)
                if (!((column->Flags | prev_column->Flags) & HvkGuiTableColumnFlags_NoReorder))
                    if ((column->IndexWithinEnabledSet < table->FreezeColumnsRequest) == (prev_column->IndexWithinEnabledSet < table->FreezeColumnsRequest))
                        table->ReorderColumnDir = -1;
        if (g.IO.MouseDelta.x > 0.0f && g.IO.MousePos.x > cell_r.Max.x)
            if (HvkGuiTableColumn* next_column = (column->NextEnabledColumn != -1) ? &table->Columns[column->NextEnabledColumn] : NULL)
                if (!((column->Flags | next_column->Flags) & HvkGuiTableColumnFlags_NoReorder))
    ...
    if ((table->Flags & HvkGuiTableFlags_Sortable) && !(column->Flags & HvkGuiTableColumnFlags_NoSort))
    {
        if (column->SortOrder != -1)
        {
            ...
            if (column->SortOrder > 0)
            {
                PushStyleColor(HvkGuiCol_Text, GetColorU32(HvkGuiCol_Text, 0.70f));
                RenderText(HvkVec2(x + g.Style.ItemInnerSpacing.x, y), sort_order_suf);
                PopStyleColor();
                x += w_sort_text;
            }
            RenderArrow(window->DrawList, HvkVec2(x, y), GetColorU32(HvkGuiCol_Text), column->SortDirection == HvkGuiSortDirection_Ascending ? HvkGuiDir_Up : HvkGuiDir_Down, ARROW_SCALE);
        }

        // Handle clicking on column header to adjust Sort Order
        if (pressed && table->ReorderColumn != column_n)
        {
            HvkGuiSortDirection sort_direction = TableGetColumnNextSortDirection(column);
...
void HvkGui::TableAngledHeadersRow()
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTable* table = g.CurrentTable;
    HvkGuiTableTempData* temp_data = table->TempData;
    ...
    const HvkGuiID row_id = GetID("##AngledHeaders");
    HvkGuiTableInstanceData* table_instance = TableGetInstanceData(table, table->InstanceCurrent);
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        if (Hvk_BITARRAY_TESTBIT(table->EnabledMaskByDisplayOrder, order_n))
        {
            const int column_n = table->DisplayOrderToIndex[order_n];
            HvkGuiTableColumn* column = &table->Columns[column_n];
            if ((column->Flags & HvkGuiTableColumnFlags_AngledHeader) == 0) // Note: can't rely on HvkGuiTableColumnFlags_IsVisible test here.
                continue;
            HvkGuiTableHeaderData request = { (HvkGuiTableColumnIdx)column_n, col_text, col_header_bg, (column_n == highlight_column_n) ? GetColorU32(HvkGuiCol_Header) : 0 };
...
// Hvkportant: data must be fed left to right
void HvkGui::TableAngledHeadersRowEx(HvkGuiID row_id, float angle, float max_label_width, const HvkGuiTableHeaderData* data, int data_count)
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTable* table = g.CurrentTable;
    HvkGuiWindow* window = g.CurrentWindow;
    ...
    const float row_height = HvkTrunc(HvkFabs(HvkRotate(HvkVec2(max_label_width, flip_label ? +header_height : -header_height), cos_a, sin_a).y));
    table->AngledHeadersHeight = row_height;
    table->AngledHeadersSlope = (sin_a != 0.0f) ? (cos_a / sin_a) : 0.0f;
    const HvkVec2 header_angled_vector = unit_right * (row_height / -sin_a); // vector from bottom-left to top-left, and from bottom-right to top-right

    // Declare row, override and draw our own background
    TableNextRow(HvkGuiTableRowFlags_Headers, row_height);
    TableNextColumn();
    const HvkRect row_r(table->WorkRect.Min.x, table->BgClipRect.Min.y, table->WorkRect.Max.x, table->RowPosY2);
    ...
    PushClipRect(HvkVec2(clip_rect_min_x, table->BgClipRect.Min.y), table->BgClipRect.Max, true); // Span all columns
    ...
    for (int pass = 0; pass < 2; pass++)
        ...
        {
            const HvkGuiTableHeaderData* request = &data[order_n];
            const int column_n = request->Index;
            HvkGuiTableColumn* column = &table->Columns[column_n];

            HvkVec2 bg_shape[4];
            bg_shape[0] = HvkVec2(column->MaxX, row_r.Max.y);
            bg_shape[1] = HvkVec2(column->MinX, row_r.Max.y);
            ...
            if (pass == 0)
            {
                ...
                const int label_lines = HvkTextCountLines(label_name, label_name_end);
                ...
                column->ContentMaxXHeadersUsed = column->ContentMaxXHeadersIdeal = column->WorkMinX + HvkCeil(label_lines * line_off_step_x - line_off_for_align_x);
                ...
                while (label_name < label_name_end)
                {
                    ...
                    HvkVec2 pivot_in = HvkVec2(window->ClipRect.Min.x - vertical_offset, window->ClipRect.Min.y + label_size.y);
                    HvkVec2 pivot_out = HvkVec2(column->WorkMinX, row_r.Max.y);
...
void HvkGui::TableOpenContextMenu(int column_n)
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTable* table = g.CurrentTable;
    ...
    if (table->Flags & (HvkGuiTableFlags_Resizable | HvkGuiTableFlags_Reorderable | HvkGuiTableFlags_Hideable))
    {
        ...
        table->ContextPopupColumn = (HvkGuiTableColumnIdx)column_n;
        table->InstanceInteracted = table->InstanceCurrent;
        const HvkGuiID context_menu_id = HvkHashStr("##ContextMenu", 0, table->ID);
        OpenPopupEx(context_menu_id, HvkGuiPopupFlags_None);
    }
}

bool HvkGui::TableBeginContextMenuPopup(HvkGuiTable* table)
{
    if (!table->IsContextPopupOpen || table->InstanceCurrent != table->InstanceInteracted)
        return false;
    const HvkGuiID context_menu_id = HvkHashStr("##ContextMenu", 0, table->ID);
    if (BeginPopupEx(context_menu_id, HvkGuiWindowFlags_AlwaysAutoResize | HvkGuiWindowFlags_NoTitleBar | HvkGuiWindowFlags_NoSavedSettings))
        return true;
...
// - HvkGuiTableFlags_Resizable   -> display Sizing menu items
// - HvkGuiTableFlags_Reorderable -> display "Reset Order"
////- HvkGuiTableFlags_Sortable   -> display sorting options (disabled)
// - HvkGuiTableFlags_Hideable    -> display columns visibility menu items
// It means if you have a custom context menus you can call this section and omit some sections, and add your own.
void HvkGui::TableDrawDefaultContextMenu(HvkGuiTable* table, HvkGuiTableFlags flags_for_section_to_display)
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
    ...
    HvkGuiTableColumn* column = (column_n != -1) ? &table->Columns[column_n] : NULL;

    // Sizing
    if (flags_for_section_to_display & HvkGuiTableFlags_Resizable)
    {
        if (column != NULL)
        {
            const bool can_resize = !(column->Flags & HvkGuiTableColumnFlags_NoResize) && column->IsEnabled;
            if (MenuItem(LocalizeGetMsg(HvkGuiLocKey_TableSizeOne), NULL, false, can_resize)) // "###SizeOne"
                TableSetColumnWidthAutoSingle(table, column_n);
        }

        const char* size_all_desc;
        if (table->ColumnsEnabledFixedCount == table->ColumnsEnabledCount && (table->Flags & HvkGuiTableFlags_SizingMask_) != HvkGuiTableFlags_SizingFixedSame)
            size_all_desc = LocalizeGetMsg(HvkGuiLocKey_TableSizeAllFit);        // "###SizeAll" All fixed
        else
            size_all_desc = LocalizeGetMsg(HvkGuiLocKey_TableSizeAllDefault);    // "###SizeAll" All stretch or mixed
    ...
    // Ordering
    if (flags_for_section_to_display & HvkGuiTableFlags_Reorderable)
    {
        if (MenuItem(LocalizeGetMsg(HvkGuiLocKey_TableResetOrder), NULL, false, !table->IsDefaultDisplayOrder))
            table->IsResetDisplayOrderRequest = true;
        want_separator = true;
    }
...
#if 0
    if ((flags_for_section_to_display & HvkGuiTableFlags_Sortable) && column != NULL && (column->Flags & HvkGuiTableColumnFlags_NoSort) == 0)
    {
        if (want_separator)
            Separator();
        want_separator = true;
        ...
        if (MenuItem("Sort in Ascending Order", NULL, column->SortOrder != -1 && column->SortDirection == HvkGuiSortDirection_Ascending, (column->Flags & HvkGuiTableColumnFlags_NoSortAscending) == 0))
            TableSetColumnSortDirection(table, column_n, HvkGuiSortDirection_Ascending, append_to_sort_specs);
        if (MenuItem("Sort in Descending Order", NULL, column->SortOrder != -1 && column->SortDirection == HvkGuiSortDirection_Descending, (column->Flags & HvkGuiTableColumnFlags_NoSortDescending) == 0))
            TableSetColumnSortDirection(table, column_n, HvkGuiSortDirection_Descending, append_to_sort_specs);
    }
#endif

    // Hiding / Visibility
    if (flags_for_section_to_display & HvkGuiTableFlags_Hideable)
    {
        if (want_separator)
            Separator();
        want_separator = true;

        PushItemFlag(HvkGuiItemFlags_AutoClosePopups, false);
        for (int other_column_n = 0; other_column_n < table->ColumnsCount; other_column_n++)
        {
            HvkGuiTableColumn* other_column = &table->Columns[other_column_n];
            if (other_column->Flags & HvkGuiTableColumnFlags_Disabled)
                continue;
            ...
            bool menu_item_active = (other_column->Flags & HvkGuiTableColumnFlags_NoHide) ? false : true;
...
{
    return sizeof(HvkGuiTableSettings) + (size_t)columns_count * sizeof(HvkGuiTableColumnSettings);
}

HvkGuiTableSettings* HvkGui::TableSettingsCreate(HvkGuiID id, int columns_count)
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTableSettings* settings = g.SettingsTables.alloc_chunk(TableSettingsCalcChunkSize(columns_count));
    TableSettingsInit(settings, id, columns_count, columns_count);
    return settings;
}

// Find existing settings
HvkGuiTableSettings* HvkGui::TableSettingsFindByID(HvkGuiID id)
{
...
HvkGuiTableSettings* HvkGui::TableGetBoundSettings(HvkGuiTable* table)
{
...
void HvkGui::TableResetSettings(HvkGuiTable* table)
{
    table->IsInitializing = table->IsSettingsDirty = true;
    table->IsResetAllRequest = false;
    table->IsSettingsRequestLoad = false;                   // Don't reload from ini
    table->SettingsLoadedFlags = HvkGuiTableFlags_None;      // Mark as nothing loaded so our initialized data becomes authoritative
}

void HvkGui::TableSaveSettings(HvkGuiTable* table)
{
    table->IsSettingsDirty = false;
    if (table->Flags & HvkGuiTableFlags_NoSavedSettings)
        return;

    // Bind or create settings data
    HvkGuiContext& g = *GHvkGui;
    HvkGuiTableSettings* settings = TableGetBoundSettings(table);
    if (settings == NULL)
    {
    ...
    settings->SaveFlags = HvkGuiTableFlags_None;
    for (int n = 0; n < table->ColumnsCount; n++, column++, column_settings++)
    {
        const float width_or_weight = (column->Flags & HvkGuiTableColumnFlags_WidthStretch) ? column->StretchWeight : column->WidthRequest;
        column_settings->WidthOrWeight = width_or_weight;
        column_settings->Index = (HvkGuiTableColumnIdx)n;
        column_settings->DisplayOrder = column->DisplayOrder;
        column_settings->SortOrder = column->SortOrder;
        column_settings->SortDirection = column->SortDirection;
        column_settings->IsEnabled = column->IsUserEnabled;
        column_settings->IsStretch = (column->Flags & HvkGuiTableColumnFlags_WidthStretch) ? 1 : 0;
        if ((column->Flags & HvkGuiTableColumnFlags_WidthStretch) == 0)
            save_ref_scale = true;
        ...
        if (width_or_weight != column->InitStretchWeightOrWidth)
            settings->SaveFlags |= HvkGuiTableFlags_Resizable;
        if (column->DisplayOrder != n)
            settings->SaveFlags |= HvkGuiTableFlags_Reorderable;
        if (column->SortOrder != -1)
            settings->SaveFlags |= HvkGuiTableFlags_Sortable;
        if (column->IsUserEnabled != ((column->Flags & HvkGuiTableColumnFlags_DefaultHide) == 0))
            settings->SaveFlags |= HvkGuiTableFlags_Hideable;
    }
...
void HvkGui::TableLoadSettings(HvkGuiTable* table)
{
    HvkGuiContext& g = *GHvkGui;
    ...
    // Serialize HvkGuiTableSettings/HvkGuiTableColumnSettings into HvkGuiTable/HvkGuiTableColumn
    HvkGuiTableColumnSettings* column_settings = settings->GetColumnSettings();
    for (int data_n = 0; data_n < settings->ColumnsCount; data_n++, column_settings++)
    {
        ...
        HvkGuiTableColumn* column = &table->Columns[column_n];
        if (settings->SaveFlags & HvkGuiTableFlags_Resizable)
        {
        ...
        if (settings->SaveFlags & HvkGuiTableFlags_Reorderable)
            column->DisplayOrder = column_settings->DisplayOrder;
        if ((settings->SaveFlags & HvkGuiTableFlags_Hideable) && column_settings->IsEnabled != -1)
...
struct HvkGuiTableFixDisplayOrderColumnData
{
    HvkGuiTableColumnIdx     Idx;
    HvkGuiTable*             Table;  // This is unfortunate but we don't have userdata in qsort api.
};

// Sort by DisplayOrder and then Index
static int HvkGui_CDECL TableFixDisplayOrderComparer(const void* lhs, const void* rhs)
{
    const HvkGuiTable* table = ((const HvkGuiTableFixDisplayOrderColumnData*)lhs)->Table;
    const HvkGuiTableColumnIdx lhs_idx = ((const HvkGuiTableFixDisplayOrderColumnData*)lhs)->Idx;
    const HvkGuiTableColumnIdx rhs_idx = ((const HvkGuiTableFixDisplayOrderColumnData*)rhs)->Idx;
...
void HvkGui::TableFixDisplayOrder(HvkGuiTable* table)
{
    HvkGuiContext& g = *GHvkGui;
    g.TempBuffer.reserve((int)(sizeof(HvkGuiTableFixDisplayOrderColumnData) * table->ColumnsCount)); // FIXME: Maybe wrap those two lines as a helper.
    HvkGuiTableFixDisplayOrderColumnData* fdo_columns = (HvkGuiTableFixDisplayOrderColumnData*)g.TempBuffer.Data;
    for (int n = 0; n < table->ColumnsCount; n++)
    {
        fdo_columns[n].Idx = (HvkGuiTableColumnIdx)n;
        fdo_columns[n].Table = table;
    }
    HvkQsort(fdo_columns, (size_t)table->ColumnsCount, sizeof(HvkGuiTableFixDisplayOrderColumnData), TableFixDisplayOrderComparer);
    for (int n = 0; n < table->ColumnsCount; n++)
        table->Columns[fdo_columns[n].Idx].DisplayOrder = (HvkGuiTableColumnIdx)n;
}

static void TableSettingsHandler_ClearAll(HvkGuiContext* ctx, HvkGuiSettingsHandler*)
{
    HvkGuiContext& g = *ctx;
    for (int i = 0; i != g.Tables.GetMapSize(); i++)
        if (HvkGuiTable* table = g.Tables.TryGetMapData(i))
            table->SettingsOffset = -1;
    g.SettingsTables.clear();
}

// Apply to existing windows (if any)
static void TableSettingsHandler_ApplyAll(HvkGuiContext* ctx, HvkGuiSettingsHandler*)
{
    HvkGuiContext& g = *ctx;
    for (int i = 0; i != g.Tables.GetMapSize(); i++)
        if (HvkGuiTable* table = g.Tables.TryGetMapData(i))
...
static void* TableSettingsHandler_ReadOpen(HvkGuiContext*, HvkGuiSettingsHandler*, const char* name)
{
    HvkGuiID id = 0;
    int columns_count = 0;
    ...
    if (HvkGuiTableSettings* settings = HvkGui::TableSettingsFindByID(id))
    {
    ...
    return HvkGui::TableSettingsCreate(id, columns_count);
}

static void TableSettingsHandler_ReadLine(HvkGuiContext*, HvkGuiSettingsHandler*, void* entry, const char* line)
{
    // "Column 0  UserID=0x42AD2D21 Width=100 Visible=1 Order=0 Sort=0v"
    HvkGuiTableSettings* settings = (HvkGuiTableSettings*)entry;
    float f = 0.0f;
    ...
    if (sscanf(line, "Column %d%n", &column_n, &r) == 1)
    {
        if (column_n < 0 || column_n >= settings->ColumnsCount)
            return;
        line = HvkStrSkipBlank(line + r);
        char c = 0;
        HvkGuiTableColumnSettings* column = settings->GetColumnSettings() + column_n;
        column->Index = (HvkGuiTableColumnIdx)column_n;
        if (sscanf(line, "UserID=0x%08X%n", (HvkU32*)&n, &r)==1) { line = HvkStrSkipBlank(line + r); column->UserID = (HvkGuiID)n; }
        if (sscanf(line, "Width=%d%n", &n, &r) == 1)            { line = HvkStrSkipBlank(line + r); column->WidthOrWeight = (float)n; column->IsStretch = 0; settings->SaveFlags |= HvkGuiTableFlags_Resizable; }
        if (sscanf(line, "Weight=%f%n", &f, &r) == 1)           { line = HvkStrSkipBlank(line + r); column->WidthOrWeight = f; column->IsStretch = 1; settings->SaveFlags |= HvkGuiTableFlags_Resizable; }
        if (sscanf(line, "Visible=%d%n", &n, &r) == 1)          { line = HvkStrSkipBlank(line + r); column->IsEnabled = (HvkU8)n; settings->SaveFlags |= HvkGuiTableFlags_Hideable; }
        if (sscanf(line, "Order=%d%n", &n, &r) == 1)            { line = HvkStrSkipBlank(line + r); column->DisplayOrder = (HvkGuiTableColumnIdx)n; settings->SaveFlags |= HvkGuiTableFlags_Reorderable; }
        if (sscanf(line, "Sort=%d%c%n", &n, &c, &r) == 2)       { line = HvkStrSkipBlank(line + r); column->SortOrder = (HvkGuiTableColumnIdx)n; column->SortDirection = (c == '^') ? HvkGuiSortDirection_Descending : HvkGuiSortDirection_Ascending; settings->SaveFlags |= HvkGuiTableFlags_Sortable; }
    }
}

static void TableSettingsHandler_WriteAll(HvkGuiContext* ctx, HvkGuiSettingsHandler* handler, HvkGuiTextBuffer* buf)
{
    HvkGuiContext& g = *ctx;
    for (HvkGuiTableSettings* settings = g.SettingsTables.begin(); settings != NULL; settings = g.SettingsTables.next_chunk(settings))
    {
        ...
        const bool save_size    = (settings->SaveFlags & HvkGuiTableFlags_Resizable) != 0;
        const bool save_visible = (settings->SaveFlags & HvkGuiTableFlags_Hideable) != 0;
        const bool save_order   = (settings->SaveFlags & HvkGuiTableFlags_Reorderable) != 0;
        const bool save_sort    = (settings->SaveFlags & HvkGuiTableFlags_Sortable) != 0;
        ...
        HvkGuiTableColumnSettings* column = settings->GetColumnSettings();
        for (int column_n = 0; column_n < settings->ColumnsCount; column_n++, column++)
        {
            ...
            if (save_sort && column->SortOrder != -1)   { buf->appendf(" Sort=%d%c", column->SortOrder, (column->SortDirection == HvkGuiSortDirection_Ascending) ? 'v' : '^'); }
            buf->append("\n");
        }
        buf->append("\n");
    }
...
void HvkGui::TableSettingsAddSettingsHandler()
{
    HvkGuiSettingsHandler ini_handler;
    ini_handler.TypeName = "Table";
    ini_handler.TypeHash = HvkHashStr("Table");
...
void HvkGui::TableRemove(HvkGuiTable* table)
{
    //HvkGui_DEBUG_PRINT("TableRemove() id=0x%08X\n", table->ID);
    HvkGuiContext& g = *GHvkGui;
    int table_idx = g.Tables.GetIndex(table);
    //memset(table->RawData.Data, 0, table->RawData.size_in_bytes());
    //memset(table, 0, sizeof(HvkGuiTable));
    g.Tables.Remove(table->ID, table);
    g.TablesLastTimeActive[table_idx] = -1.0f;
}

// Free up/compact internal Table buffers for when it gets unused
void HvkGui::TableGcCompactTransientBuffers(HvkGuiTable* table)
{
...
void HvkGui::TableGcCompactTransientBuffers(HvkGuiTableTempData* temp_data)
{
...
void HvkGui::TableGcCompactSettings()
{
    HvkGuiContext& g = *GHvkGui;
    ...
    if (required_memory == g.SettingsTables.Buf.Size)
        return;
    HvkChunkStream<HvkGuiTableSettings> new_chunk_stream;
...
static const char* DebugNodeTableGetSizingPolicyDesc(HvkGuiTableFlags sizing_policy)
{
    sizing_policy &= HvkGuiTableFlags_SizingMask_;
    if (sizing_policy == HvkGuiTableFlags_SizingFixedFit)    { return "FixedFit"; }
    if (sizing_policy == HvkGuiTableFlags_SizingFixedSame)   { return "FixedSame"; }
    if (sizing_policy == HvkGuiTableFlags_SizingStretchProp) { return "StretchProp"; }
    if (sizing_policy == HvkGuiTableFlags_SizingStretchSame) { return "StretchSame"; }
    return "N/A";
}

void HvkGui::DebugNodeTable(HvkGuiTable* table)
{
    HvkGuiContext& g = *GHvkGui;
    ...
    {
        HvkGuiTableInstanceData* table_instance = TableGetInstanceData(table, n);
    ...
    for (int n = 0; n < table->ColumnsCount; n++)
        if (table->Columns[n].Flags & HvkGuiTableColumnFlags_WidthStretch)
    ...
    for (int n = 0; n < table->ColumnsCount; n++)
    {
        ...
        HvkFormatString(buf, Hvk_ARRAYSIZE(buf),
            ...
            column->SortOrder, (column->SortDirection == HvkGuiSortDirection_Ascending) ? " (Asc)" : (column->SortDirection == HvkGuiSortDirection_Descending) ? " (Des)" : "", column->UserID, column->Flags,
            (column->Flags & HvkGuiTableColumnFlags_WidthStretch) ? "WidthStretch " : "",
            (column->Flags & HvkGuiTableColumnFlags_WidthFixed) ? "WidthFixed " : "",
            (column->Flags & HvkGuiTableColumnFlags_NoResize) ? "NoResize " : "");
        Bullet();
        Selectable(buf);
        if (IsItemHovered())
        {
...
void HvkGui::DebugNodeTableSettings(HvkGuiTableSettings* settings)
{
    ...
    {
        HvkGuiTableColumnSettings* column_settings = &settings->GetColumnSettings()[n];
        HvkGuiSortDirection sort_dir = (column_settings->SortOrder != -1) ? (HvkGuiSortDirection)column_settings->SortDirection : HvkGuiSortDirection_None;
        BulletText("Column %d Order %d SortOrder %d %s Vis %d %s %7.3f UserID 0x%08X",
            n, column_settings->DisplayOrder, column_settings->SortOrder,
            (sort_dir == HvkGuiSortDirection_Ascending) ? "Asc" : (sort_dir == HvkGuiSortDirection_Descending) ? "Des" : "---",
            column_settings->IsEnabled, column_settings->IsStretch ? "Weight" : "Width ", column_settings->WidthOrWeight, column_settings->UserID);
    }
    TreePop();
}

#else // #ifndef HvkGui_DISABLE_DEBUG_TOOLS

void HvkGui::DebugNodeTable(HvkGuiTable*) {}
void HvkGui::DebugNodeTableSettings(HvkGuiTableSettings*) {}
...
// they would meddle many times with the underlying HvkDrawCmd.
// Instead, we do a preemptive overwrite of clipping rectangle _without_ altering the command-buffer and let
// the subsequent single call to SetCurrentChannel() does it things once.
void HvkGui::SetWindowClipRectBeforeSetChannel(HvkGuiWindow* window, const HvkRect& clip_rect)
{
    HvkVec4 clip_rect_vec4 = clip_rect.ToVec4();
    window->ClipRect = clip_rect;
    window->DrawList->_CmdHeader.ClipRect = clip_rect_vec4;
    window->DrawList->_ClipRectStack.Data[window->DrawList->_ClipRectStack.Size - 1] = clip_rect_vec4;
}

int HvkGui::GetColumnIndex()
{
    HvkGuiWindow* window = GetCurrentWindowRead();
    return window->DC.CurrentColumns ? window->DC.CurrentColumns->Current : 0;
}

int HvkGui::GetColumnsCount()
{
    HvkGuiWindow* window = GetCurrentWindowRead();
    return window->DC.CurrentColumns ? window->DC.CurrentColumns->Count : 1;
}

float HvkGui::GetColumnOffsetFromNorm(const HvkGuiOldColumns* columns, float offset_norm)
{
    return offset_norm * (columns->OffMaxX - columns->OffMinX);
}

float HvkGui::GetColumnNormFromOffset(const HvkGuiOldColumns* columns, float offset)
{
...
static float GetDraggedColumnOffset(HvkGuiOldColumns* columns, int column_index)
{
...
float HvkGui::GetColumnOffset(int column_index)
{
    HvkGuiWindow* window = GetCurrentWindowRead();
...
static float GetColumnWidthEx(HvkGuiOldColumns* columns, int column_index, bool before_resize = false)
{
    if (column_index < 0)
        column_index = columns->Current;
    ...
    return HvkGui::GetColumnOffsetFromNorm(columns, offset_norm);
}

float HvkGui::GetColumnWidth(int column_index)
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiWindow* window = g.CurrentWindow;
    HvkGuiOldColumns* columns = window->DC.CurrentColumns;
...
void HvkGui::SetColumnOffset(int column_index, float offset)
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiWindow* window = g.CurrentWindow;
    HvkGuiOldColumns* columns = window->DC.CurrentColumns;
...
void HvkGui::PushColumnClipRect(int column_index)
{
    HvkGuiWindow* window = GetCurrentWindowRead();
    HvkGuiOldColumns* columns = window->DC.CurrentColumns;
    if (column_index < 0)
    ...
    HvkGuiOldColumnData* column = &columns->Columns[column_index];
    PushClipRect(column->ClipRect.Min, column->ClipRect.Max, false);
}

// Get into the columns background draw command (which is generally the same draw command as before we called BeginColumns)
void HvkGui::PushColumnsBackground()
{
    HvkGuiWindow* window = GetCurrentWindowRead();
    HvkGuiOldColumns* columns = window->DC.CurrentColumns;
    if (columns->Count == 1)
...
void HvkGui::PopColumnsBackground()
{
    HvkGuiWindow* window = GetCurrentWindowRead();
    HvkGuiOldColumns* columns = window->DC.CurrentColumns;
    if (columns->Count == 1)
...
HvkGuiOldColumns* HvkGui::FindOrCreateColumns(HvkGuiWindow* window, HvkGuiID id)
{
    ...
    window->ColumnsStorage.push_back(HvkGuiOldColumns());
    HvkGuiOldColumns* columns = &window->ColumnsStorage.back();
    columns->ID = id;
    return columns;
}

HvkGuiID HvkGui::GetColumnsID(const char* str_id, int columns_count)
{
    ...
    HvkGuiID id = window->GetID(str_id ? str_id : "columns");
    PopID();

    return id;
}

void HvkGui::BeginColumns(const char* str_id, int columns_count, HvkGuiOldColumnFlags flags)
{
    HvkGuiContext& g = *GHvkGui;
    HvkGuiWindow* window = GetCurrentWindow();
...
void HvkGui::NextColumn()
{
    ...
    HvkGuiOldColumnData* column = &columns->Columns[columns->Current];
    SetWindowClipRectBeforeSetChannel(window, column->ClipRect);
    columns->Splitter.SetCurrentChannel(window->DrawList, columns->Current + 1);
...
void HvkGui::EndColumns()
{
    ...
    if (!(flags & HvkGuiOldColumnFlags_NoBorder) && !window->SkipItems)
    {
        ...
        for (int n = 1; n < columns->Count; n++)
        {
            ...
            const HvkGuiID column_id = columns->ID + HvkGuiID(n);
            const float column_hit_hw = HvkTrunc(COLUMNS_HIT_RECT_HALF_THICKNESS * g.CurrentDpiScale);
            const HvkRect column_hit_rect(HvkVec2(x - column_hit_hw, y1), HvkVec2(x + column_hit_hw, y2));
            if (!ItemAdd(column_hit_rect, column_id, NULL, HvkGuiItemFlags_NoNav))
                continue;

            bool hovered = false, held = false;
            if (!(flags & HvkGuiOldColumnFlags_NoResize))
            {
                ButtonBehavior(column_hit_rect, column_id, &hovered, &held);
                if (hovered || held)
                    SetMouseCursor(HvkGuiMouseCursor_ResizeEW);
                if (held && !(column->Flags & HvkGuiOldColumnFlags_NoResize))
                    dragging_column = n;
            }
